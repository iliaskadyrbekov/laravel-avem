...
@section('content')
    <div class="city-info">
        <div class="info__poster" style="background-image: linear-gradient(0deg, rgba(255, 255, 255, 0) 0%, rgba(0, 0, 0, 0.3) 100%), url(data:image/jpg;base64,{{$city->background_image}})">
            <div class="poster__country">
                <p>{{$city->country->country_name}}</p>
            </div>
        ...
        <div class="info__about">
            <div class="about__welcome">
                <h2>Welcome to {{$city->city_name}}</h2>
                <hr>
            </div>
        ...
        <div class="info__carusel">
            <div class="carusel__title">
                <h2>Highlights in {{$city->city_name}}</h2>
            </div>
            ...
            <div class="carusel__holder" id="carusel">
                @switch(count($highlights))
                    @case(1)
                        <a href="/place/{{$highlights[0]->id}}" class="holder__image" id="caruselUnit" style="flex: 0 0 100%; background-image: linear-gradient(180deg, rgba(255, 255, 255, 0) 0%, rgba(0, 0, 0, 0.3) 100%), url(data:image/jpg;base64,{{$highlights[0]->background_image}})">
                            <h3>{{ $highlights[0]->city->city_name ?? $city->city_name }}</h3>
                            <p>{{ $highlights[0]->place_name ?? 'empty'}}</p>
                        </a>
                        @break
                    @case(2)
                        @foreach($highlights as $place)
    ...
    <div class="posts">
        @if(count($city->blogs) > 0)
            <h2 class="posts__title">From community members<span id="publication_counter">({{count($city->blogs)}})</span>
            </h2>
        @endif

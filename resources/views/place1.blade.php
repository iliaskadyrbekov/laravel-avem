...
@section('content')
    <div class="place-info">
        <div class="info__poster"
             style="background-image: linear-gradient(0deg, rgba(255, 255, 255, 0) 0%, rgba(0, 0, 0, 0.3) 100%), url(data:image/jpg;base64,{{$place->background_image}})">
            <div class="poster__city">
                <a href="/city/{{$place->city->id ?? '#'}}"><p>{{$place->city->city_name}}</p></a>
            </div>
            <div class="poster__place">
        ...
        <div class="info__details">
            @if(!empty($place->address) || !empty($place->phone_number) || !empty($place->website) || !empty($place->working_hours))
                <div class="details__title">
                    <h2>Details</h2>
                </div>
            @endif
            @if(!empty($place->address))
                <div class="details__location"><p>{{$place->address ?? ''}}</p></div>
            @endif
            @if(!empty($place->phone_number))
                <div class="details__phone"><p>{{$place->phone_number ?? ''}}</p></div>
            @endif
            @if(!empty($place->website))
                <div class="details__site"><a href="{{$place->website ?? '#'}}">Visit website</a></div>
            @endif
            @if(!empty($place->working_hours))
                <div class="details__hours">
                    <p>Hours:</p>
                    <table>
                        @foreach(json_decode($place->working_hours) ?? [] as $key => $value)
                            <tr>
                                <th>{{$weekArr[$key]}}</th>
                                <td>{{$value}}</td>
                            </tr>
    ...
    <div class="posts">
        @if(count($place->blogs) > 0)
            <h2 class="posts__title">Publications with {{$place->place_name}}<span id="publication_counter">({{count($place->blogs)}})</span>
            </h2>
        @endif
                    ...
                    <div class="post__feedback">
                        ...
                        <div class="post__share">
                            <a class="post__share-link"
                               onclick='copyToClipboard(window.location.hostname + "/blog/{{ $blog->id }}")' href="#">
        ...
        @endforeach
        @if($content['hasMore'])
            <a href="/city/{{ $city->id.'/'.$content['pages'] }}">
